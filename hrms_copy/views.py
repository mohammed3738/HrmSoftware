...
def create_company(request):
    if request.method == "POST":
        ...
        return redirect("create_company")  # Redirect to company list page
    companies = Company.objects.all()
...
def leave_balance_view(request):
    ...
    return render(request, "leave_balance/leave_balance2.html", {"leave_balances": leave_balances})
...
@csrf_exempt  # Temporary for debugging; remove later
def submit_comp_off_request(request):
    if request.method == "POST":
        try:
            print("Request Body:", request.body)  # Log raw request body
            print("POST Data:", request.POST)  # Log parsed POST data
...
def advance_list(request):
    advances = AdvanceMaster.objects.all().select_related('employee')
    employees = Employee.objects.all()
...
def create_advance(request):
    if request.method == "POST":
        ...
        start_month = datetime.strptime(request.POST.get("start_month"), "%Y-%m-%d").date()
        remarks = request.POST.get("remarks", "")

        advance = AdvanceMaster.objects.create(
            employee_id=emp_id,
        ...
        installment_amount = total_amount / total_months
...
@csrf_exempt
def toggle_installment(request, installment_id):
    """Toggle deduction for a particular month"""
    installment = get_object_or_404(AdvanceInstallment, id=installment_id)
    installment.is_deducted = not installment.is_deducted
    installment.save()
    return JsonResponse({"status": "success", "deducted": installment.is_deducted})
...
#     installment.paid_on = timezone.now()
#     installment.save()

#     # Update AdvanceMaster totals
#     advance = installment.advance
#     advance.paid_amount = sum(i.amount for i in advance.installments.filter(is_paid=True))
#     advance.remaining_amount = advance.total_amount - advance.paid_amount
#     advance.save()

#     return JsonResponse({"message": "Installment marked as paid!", "paid": True})


@csrf_exempt
def mark_paid(request, installment_id):
    """
    Toggle installment as paid/unpaid and recalculate total paid amount dynamically.
    """
    installment = get_object_or_404(AdvanceInstallment, id=installment_id)
    advance = installment.advance

    # Toggle state (undo supported)
    installment.is_paid = not installment.is_paid
    installment.save()

    # Dynamically compute new totals (no need to assign to AdvanceMaster)
    paid_sum = advance.installments.filter(is_paid=True).aggregate(
        total=Sum('amount')
    )['total'] or 0
    total_sum = advance.installments.aggregate(total=Sum('amount'))['total'] or 0
    remaining = total_sum - paid_sum

    # Prepare a small progress snapshot for the frontend
    data = {
        "installment_id": installment.id,
        "is_paid": installment.is_paid,
        "paid_amount": round(paid_sum, 2),
        "remaining_amount": round(remaining, 2),
        "progress": f"{(paid_sum / total_sum * 100):.2f}%" if total_sum else "0%",
    }
...
def undo_paid(request, installment_id):
    """Undo a mistaken 'paid' action."""
    installment = get_object_or_404(AdvanceInstallment, id=installment_id)
    installment.is_paid = False
    installment.paid_on = None
    installment.save()

    # Update totals
    advance = installment.advance
    total_paid = sum(i.amount for i in advance.installments.filter(is_paid=True))
    remaining = advance.total_amount - total_paid

    advance.paid_amount_db = total_paid
    advance.remaining_amount_db = remaining
    advance.save(update_fields=["paid_amount_db", "remaining_amount_db"])

    return JsonResponse({
        "message": "Installment reverted to pending.",
        "paid": False,
...
def get_installments(request, advance_id):
    """Return HTML for the installments modal dynamically"""
    advance = get_object_or_404(AdvanceMaster, id=advance_id)
    installments = advance.installments.order_by("month")
    html = render_to_string("advance/installment_table.html", {"advance": advance, "installments": installments})
